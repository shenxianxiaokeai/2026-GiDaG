...
from gidag.data import load_email_eu_pt
...
def _summarize(rows: List[Dict]) -> List[Dict]:
    ...
    for (dataset, method), items in grouped.items():
        def stat(key):
            values = np.asarray([float(x[key]) for x in items], dtype=np.float64)
...
def run_email_eu_experiment(config: ExperimentConfig) -> Dict:
    ...
    graph = load_email_eu_pt(config.dataset_path)
    ...
    for run_id in range(config.num_runs):
        ...
        if "gee" in methods or "gidag" in methods or "gidag-c" in methods:
            st = time.time()
        ...
        if "gnn" in methods:
            st = time.time()
            gnn_output = pipeline.run_gnn_random(graph)
            elapsed = time.time() - st
            metrics = all_metrics(y_true_np, gnn_output["pred_labels"], graph.num_nodes, graph.edges_undirected)
            record_artifact(
                method_name="GNN",
                run_id=run_id,
                seed=seed,
        ...
        if "gidag" in methods:
            ...
            elapsed = time.time() - st
            metrics = all_metrics(y_true_np, gidag_output["pred_labels"], graph.num_nodes, graph.edges_undirected)
            record_artifact(
                method_name="GiDaG",
                run_id=run_id,
                seed=seed,
                y_pred=gidag_output["pred_labels"],
                embeddings=gidag_output["embeddings"],
                metrics=metrics,
                elapsed=elapsed,
                run_payload=run_payload,
            )
        ...
        if "gidag-c" in methods:
            ...
            elapsed = time.time() - st
            metrics = all_metrics(y_true_np, gidag_c_output["pred_labels"], graph.num_nodes, graph.edges_undirected)
            record_artifact(
                method_name="GiDaG-C",
                run_id=run_id,
                seed=seed,
        ...
        for row in run_rows:
            print(
                "  {m:8s} | NMI={nmi:.4f} ACC={acc:.4f} ARI={ari:.4f} Q={q:.4f}".format(
                    m=row["method"],
                    nmi=row["NMI"],
                    acc=row["ACC"],
                    ari=row["ARI"],
