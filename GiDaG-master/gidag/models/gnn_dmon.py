...
def _predict_from_embeddings(z: torch.Tensor, num_clusters: int, head: str) -> np.ndarray:
    z_np = z.detach().cpu().numpy()
    if head == "argmax":
        return np.argmax(z_np, axis=1).astype(np.int64)
    if head == "kmeans":
...
def train_gnn_dmon(
    ...
    model = ResidualGCN(in_dim=in_dim, hidden_dim=hidden_dim, out_dim=num_clusters, dropout=dropout).to(device)
    ...
    with torch.no_grad():
        z = model(x, edge_index)
    y_pred = _predict_from_embeddings(z, num_clusters=num_clusters, head=cluster_head)
